...
At one point or another when working with an ARM Cortex-M MCU, you will hit something like:

```shell
(gdb) backtrace
#0  HardFault_Handler() at startup.c:136
...
If you are trying to debug a Cortex-M0, you can skip ahead to [this section](#registers-prior-to-exception) where we discuss how to recover the core register state and instruction being executed at the time of the exception.
...
- `NOCP` - Indicates that a Cortex-M coprocess instruction was issued but the coprocessor is disabled or not present. One common case where this fault happens code is compiled to use the Floating Point extension (`-mfloat-abi=hard` `-mfpu=fpv4-sp-d16`) but the coprocessor was not [enabled](https://interrupt.memfault.com/blog/cortex-m-rtos-context-switching#fpu-config-options) on boot.
- `INVPC` - Indicates an integrity check failure on `EXC_RETURN` has returned. We'll explore an example [below](#coprocessor-fault-example). [`EXC_RETURN`](https://interrupt.memfault.com/blog/cortex-m-rtos-context-switching#exc-return-info) is the value branched to upon return from an exception. If this fault flag is set, it means a reserved `EXC_RETURN` value was used on exception exit.
- `INVSTATE` - Indicates the processor has tried to execute an instruction with an invalid _Execution Program Status Register_ (**EPSR**). Among other things the ESPR tracks whether or not the processor is in thumb mode state. Instructions which use "interworking addresses"[^1] (`bx` & `blx` or `ldr` & `ldm` when loading a `pc`-relative value) must set `bit[0]` of the instruction to 1 as this is used to update `ESPR.T`. If this rule is violated, a `INVSTATE` exception will be generated. When writing C code, the compiler will take care of this automatically, but this is a common bug which can arise when hand-writing assembly.
...
When a debugger is attached, lockup often has a different behavior. For example, on the NRF52840, "Reset from CPU lockup is disabled if the device is in debug interface mode"[^11].
...
Many Real Time Operating Systems (**RTOS**) targetting Cortex-M devices will add options to dump verbose fault register information to the console upon crash. Some examples include Arm Mbed OS[^2] and Zephyr[^3]. For example, with Zephyr `illegal_instruction_execution()` crash looks like:
...
One approach is to simply try and reproduce the issue on site. This is a guessing game (are you actually reproducing the customer hit?), can be a huge time sync and in some cases is not even particularly feasible[^14].
...
Now, the interesting part, how do we clean up our state and exit the HardFault handler?!
...
So we see there was a NOCP was due to issuing a FP instruction. The FPU is enabled using bits 20-23 of the [CPACR](https://interrupt.memfault.com/blog/cortex-m-rtos-context-switching#fpu-config-options) register located at `0xE000ED88`. A value of 0 indicates the extension is disabled. Let's check it:
